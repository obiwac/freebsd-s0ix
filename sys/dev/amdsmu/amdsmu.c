...
static void
amdsmu_diagnose(device_t dev, enum power_stype stype)
{
	struct amdsmu_softc *sc = device_get_softc(dev);
	struct amdsmu_metrics *m = &sc->metrics;
	...
	for (size_t i = 0; i < sc->product->ip_block_count; i++) {
		 ...
		 * for the entire time spent in sw_drips.
		 */
...
amdsmu_attach(device_t dev)
{
	...
	sc->eh_diagnose = EVENTHANDLER_REGISTER(power_resume,
	    amdsmu_diagnose, dev, EVENTHANDLER_PRI_ANY);
#endif
...
amdsmu_detach(device_t dev)
{
	...
	EVENTHANDLER_DEREGISTER(power_resume, sc->eh_diagnose);
#endif
